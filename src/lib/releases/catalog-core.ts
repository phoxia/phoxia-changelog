...
export function validateRelease(value: unknown): Release {
  ...
  for (const key of required) if (!(key in record)) throw new Error(`missing ${key}`);
  for (const key of Object.keys(record)) if (!allowed.has(key)) throw new Error(`unexpected ${key}`);
